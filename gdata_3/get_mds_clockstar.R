...
dat_raw <- as.matrix(read.table('sbsd_c1.txt', head = T, as.is = T))
...
true_dat_clus <- clus_fun_par(mds_dat, 25)
stopCluster(cl)
...
for(i in 1:50){
      ...
      boot_dat_clus[[i]] <- clus_fun_par(boot_dat_temp, 25)
}
...
par(mfrow = c(2, 1))
plot(2:25, as.numeric(true_dat_clus), type = 'l', lwd = 2, col = 'red', ylim = c(0, 1), ylab = 'Average silhouette width', xlab = 'Number of topology clusters')
for(i in 1:length(boot_dat_clus)){
      points(jitter(2:25), as.numeric(boot_dat_clus[[i]]), pch = 20, col = rgb(0, 0, 0.5, 0.2))
