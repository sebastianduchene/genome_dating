...
clock_table <- read.table('clustering_k_3.txt')
...
for(i in 1:nrow(unique(clock_table))){
      ...
      write.dna(concat_genes[[i]], file = 'concat_single_clocks.phy', append = T, nbcol = -1, colsep = '')
}


raw_lines <- readLines('concat_single_clocks.phy')
